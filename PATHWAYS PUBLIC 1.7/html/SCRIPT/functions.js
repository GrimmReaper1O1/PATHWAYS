...
let swap = (e) => {
    ...
    if (sessionStorage.getItem('swap1') === null) {
        ...
        let menu = JSON.parse(sessionStorage.getItem('menu'));
        let place1 = menu;
        sessionStorage.setItem('lastPath', sessionStorage.getItem('swap1'));
...
// console.log(obj2);      
 ...
 let arrHelper = (counter, posArr, splice, tempArr, a) => {
                            ...
                            for (let x = counter-1; x >= 0; x--) {
                                ...
                                tempArr[a].unshift(splice.link[a][x]);
                                console.log(tempArr[a]);
                            }
                      
                            splice.link[a] = tempArr[a];
         ...
         let tempArr = [];
         let tempArr2 = [];
 if (typeof splice.link !== 'undefined' && splice.link !== arr){
    if (fPath.length === 0) {
            for (let i = 0; i < splice.link.length; i++) {
                if (fPath.length === 0) {
                    if (typeof splice.link[i][0] !== 'undefined') {
                        console.log(opt);
                splice.link[i][0] = opt;
                    }
                }
        ...
        if (splice.link !== arr && typeof splice.link !== 'undefined'){
            ...
            for (let i = 0; i < splice.link.length; i++) {
                ...
                splice.link[i][0] = opt;
                    ...
                    for (let a = 0; a < splice.link.length; a++) {
                        tempArr[a] = [];
                 
                        for (let i = 0; i < splice.link[a].length; i++) {
                            console.log(splice.link[a][i]);
                        
                        tempArr[a].push(splice.link[a][i]);
                        if (fPath.length === lastPath.length) {
                            tempArr[a].pop();
                            tempArr[a].push(lastPath[lastPath.length-1]);
                        }
                    }
        ...
        for (let a = 0; a < splice.link.length; a++) {
            ...
            // debugger;
                     if (lastPath.length <= path.length && splice.link[a].length >= path.length) {
                            console.log('here');
                           for (let y = 0; y < path.length; y++) {
                                if (typeof lastPath[y] !== 'undefined' && lastPath[y] === path[y] && lastPath.length !== y) {
                                    if (stop ) {
                                        counter++
                                        stop = false;
                                    } else {
                      ...
                      splice = arrHelper(counter, posArr, splice, tempArr, a);                     
                        ...
                        } else if (lastPath.length >= path.length && splice.link[a].length >= lastPath.length) {
                        for (let y = 0; y < lastPath.length; y++) {
                                if (typeof path[y] !== 'undefined' && lastPath[y] === path[y] && path.length !== y) {
                                    if (stop) {
                                        counter++
                                        stop = false;
                                    } else {
        ...
        let nObj = {};
        for (let key in splice) {
            if (typeof splice[key] !== 'undefined' && typeof splice[key].bS !== 'undefined') {
                // console.log(splice);
                nObj = splice[key]
                nObj = findLinks(opt, nObj, fPath);
            }
