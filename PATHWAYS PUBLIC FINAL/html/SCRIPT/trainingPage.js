let reflow2 = (el) => {
    ...
    setTimeout(() => { let width = el.offsetHeight; }, 200);
};   
let pressFunction = (e, opt) => {
    ...
    sessionStorage.removeItem('linkArr2');
...
let displayImageOnScreenBreifly = (imageFileName, t) => {
    ...
    div.classList.add('align-centered');
...
let selectFunkytion = (e, opt = 'first', opt2 = 0, back = false, link = false, followed = false) => {
        ...
        let resumeObj;
            ...
            if (sessionStorage.getItem('linkArr2') !== null) {
                linkArr2 = JSON.parse(sessionStorage.getItem('linkArr2'));
            }
            ...
            if (back && !followed) {
                fPath2.pop();
                linkArr2.pop();
                if (linkArr2[linkArr2.length-1] === true) {
                    isLink = true;
                } 
        ...
        console.log(fPath2);
        console.log(linkArr2);
        console.log(opt2);
        // if (followed) {
    //    }
        sessionStorage.setItem('linkArr2', JSON.stringify(linkArr2));
        ...
        if (typeof obj.dB !== 'undefined') {
            ...
            if (obj.dIW !== '') {
                string += `<br><br><br><div id="trainingImage" class="margin-auto width-80 align-centered"><img src="./IMAGES/${obj.pFN}" class="inline image-height-100"/></div>`;
        ...
        string += `<button onclick="selectFunkytion(event, 'second', '${fPathNum}', 'true', false, false)" class="selectionButton">BACK</button><br><br>`;
        }
        if (isLink) {
            string += `<p>${paragraphReplace(resumeObj.l)}</p>`
          }  else if (followed && !back) {
               string += `<p>${paragraphReplace(resumeObj.l)}</p>`
        ...
        for (let key in obj) {
            ...
            for (let i = 0; i < links.length; i++) {
                ...
                sessionStorage.setItem(`a${i}`, JSON.stringify(links[i]));
                    ...
                    string += `Links are missing! Please redo links for this option.`;
                }
            }
        }
        ...
        let linkArr2 = JSON.parse(sessionStorage.getItem('linkArr2'));
        // linkArr2.pop();
