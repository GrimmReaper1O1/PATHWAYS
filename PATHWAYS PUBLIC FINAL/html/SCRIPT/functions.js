...
let findAndReplaceLinks = (placea, fPath, option, start = true, inplant = false) => {
  ...
  let opt = replace ? fPath[fPath.length - 1] : keyLength;
    ...
    let replaceLinks = (opt, objb, fPath, start, inplant) => {
        let linky = objb;
        let arr = [];
        let theLinkObj = {};
        ...
        if (typeof linky.link !== 'undefined') {
            ...
            if (linky.link !== arr && typeof linky.link !== 'undefined') {
                ...
                for (let a = 0; a < linky.link.length; a++) {
                    ...
                    if (linkyOfTruthArr[a] && (start ? true : (theLinkObj[linky.uid][a] ? false : true))) {
                        // // console.log('here');
        ...
        if (typeof linky === 'object') {
            ...
            for (let itemKey in linky) {
                ...
                if (typeof newerObj === 'object' && typeof newerObj.bS !== 'undefined') {
                    ...
                    linky[itemKey] = replaceLinks(opt, newerObj, fPath, start, inplant);
    ...
    nObj = replaceLinks(opt, nObj, fPath, start, inplant)
    // // console.log(nObj);
    let result = nObj;
    // console.log(result);
    return result
            ...
            if ((obj2.length + 1) < 255) {
                ...
                sessionStorage.setItem('opt', opt);
                    if (replace) {
                         ...
                         obj = findAndReplaceLinks(obj, fPath, 'option5', true, false);
                    } else {
                         obj = findAndReplaceLinks(obj, fPath, 'option5', true, true);
                ...
                for (let item in obj) {
                    if (typeof obj[item] !== 'undefined') {
                        ...
                        if (typeof obj4.bS !== 'undefined') {
                            sessionStorage.setItem('opt', opt);
                            // console.log(sessionStorage.getItem('opt'));
                            obj4 = findAndReplaceLinks(obj4, fPath, 'option5', true, true);
                            // console.log(obj4);
                            obj2[opt] = {};
                            obj4.itemKey = opt;
                            obj2[opt] = obj4;
...
let saveHelper = (option) => {
    ...
    if (option === 'option') {
        ...
        obj = findPathAndSave(place, fPath, fieldHelper(), option);
    } else {
        obj = findPathAndSave(place, fPath, fieldHelper(), option);
    }
