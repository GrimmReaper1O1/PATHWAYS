...
let modInsert = (bS) => {
    ...
    let itemKey = getExistingOptions();
    let uid = uidOne();
    obj[String(obj.length)] = {};
    obj[String(obj.length)] = { itemKey: obj.length, length: 0, 'bS': bS, 'uid': uid, cONC: 'correct', o: 'NOT REQUIRED', r: el.textContent };
...
let findPathAndSave = (fPath, infoObj, option) => {
    ...
    if (option === 'option' && typeof obj2 !== 'undefined') {
        ...
        obj2[keyLength] = {
            ...
            bS: obj.breifDescription.value, o: obj.option.value, r: obj.answerToOption.value, uid: uidOne(), cONC: 'correct'
        };
        ...
        obj2.bS = obj.breifDescription.value;
        obj2.o = obj.option.value;
        obj2.r = obj.answerToOption.value;
        obj2.uid = uidOne();
        obj2.cONC = "correct";
        return place
...
let enterInformationIntoFields = (location, truthy = true) => {
    ...
    if (truthy) {
        ...
        fieldObj.breifDescription.textContent = location.bS;
        fieldObj.option.textContent = location.o;
        fieldObj.answerToOption = location.r;
    } else {

        let el = document.getElementById('optionInfoIndividual');
        let string = `<h4>Number of seconds to display image: ${location.s !== '' ? location.s : 'NOT INSERTED'}</h4>`;
...
// remove all traces of the below
let getExistingOptions = () => {
    //     let obj = JSON.parse(sessionStorage.getItem('menu'))
    ...
    //     console.log(path);
    //     let counter = 0;
    //     for (let key1 of place) {
    //         console.log(key1);
    //         console.log(place[key1]);
    //         if (typeof place[key1].bS !== 'undefined') {
    //       counter = counter + 1;
    //         }
    // }
    return false
